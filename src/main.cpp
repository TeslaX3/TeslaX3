...
uint256 hashGenesisBlock("0xc517bd5a538156810f66c880d728bb1dfb1d35c06b2144b4561cb0381250b176");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TeslaX3: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TeslaX3 Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight < 2628000) // stops generation of more coins
	{
		nSubsidy >>= (nHeight / 262800);
	}
	if(nHeight == 1)
	{
		nSubsidy = 262500 * COIN; // 0.2% for bounties and giveaways.
	}
	if(nHeight > 2627999) // stops generation of more coins
	{
		nSubsidy = 0 * COIN;
	}	
...
static const int64 nTargetTimespan = 4 * 60;	// TeslaX3: 240 sec
static const int64 nTargetSpacing = 2 * 60;						// TeslaX3: 120 sec
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 4 * 60; // 240 Minutes
static const int64 nTargetSpacingRe = 2 * 60; // TeslaX3: 120 seconds
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal teslax3 miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TeslaX3", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x61821ee34fb559ed87b3dbbbecdb3a38dd899702552a18ef0a4a8c35c91fd95c");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Nestle recalls two kinds of Hot Pockets";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1392882134;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 223891;

        if (fTestNet)
        {
            block.nTime    = 1392866032;
            block.nNonce   = 314529;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc8bfc6f2a1932f2533db059dce7dde47b4e33b35d8327236c81b8a777248324d"));
...
{
	return false;
	//commenting out this code until we properly implement TESLA alerts.
	/*
    if (!CheckSignature())
        return false;
    if (!IsInEffect())
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // TeslaX3: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // TeslaX3d: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
